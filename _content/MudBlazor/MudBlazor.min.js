...
function stopWatchingDarkThemeMedia(){darkThemeMediaQuery.removeEventListener('change',darkModeChangeListener);}function setRippleOffset(event,target){const rect=target.getBoundingClientRect();const x=event.clientX-rect.left-rect.width/2;const y=event.clientY-rect.top-rect.height/2;target.style.setProperty("--mud-ripple-offset-x",`${x}px`);target.style.setProperty("--mud-ripple-offset-y",`${y}px`);}
...
resync(id){var existingEntry=this._maps[id];if(existingEntry){existingEntry.resync();}}
...
connect(elements,ids){var result=[];this.logger('[MudBlazor | ResizeObserver] Start observing elements...');for(var i=0;i<elements.length;i++){var newEntry={element:elements[i],id:ids[i],isInitialized:false,};this.logger("[MudBlazor | ResizeObserver] Start observing element:",{newEntry});result.push(window.mudElementRef.getBoundingClientRect(elements[i]));this._observervedElements.push(newEntry);this._resizeObserver.observe(elements[i]);}
return result;}
...
resync(){var result=[];this.logger('[MudBlazor | ResizeObserver] Resyncing...');for(var i=0;i<this._observervedElements.length;i++){var entry=this._observervedElements[i];result.push({id:entry.id,size:window.mudElementRef.getBoundingClientRect(entry.element)});}
...
click(id){const element=document.getElementById(id);if(element){element.click();}}};window.mudWindow=new MudWindow();class MudElementReference{constructor(){this.listenerId=0;this.eventListeners={};}
