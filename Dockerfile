...
WORKDIR /src
COPY ["src/BlazorProcket/BlazorProcket.csproj", "src/BlazorProcket/"]
COPY ["src/BlazorProcket.Client/BlazorProcket.Client.csproj", "src/BlazorProcket.Client/"]
...
RUN dotnet restore "./src/BlazorProcket/BlazorProcket.csproj"
COPY . .
WORKDIR "/src/src/BlazorProcket"
RUN dotnet build "./BlazorProcket.csproj" -c $BUILD_CONFIGURATION -o /app/build

FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./BlazorProcket.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "BlazorProcket.dll"]